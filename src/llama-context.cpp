...
static bool llama_model_has_cacheable_moe_weights(
    ...
    if ((int) physical_devices.size() < config.min_devices) {
        LLAMA_LOG_INFO(
                "%s: MoE cache ineligible: %d CUDA device(s) meet min CC %d.%d, need %d\n",
                __func__, (int) physical_devices.size(),
                config.min_compute_capability / 100,
                (config.min_compute_capability % 100) / 10,
                config.min_devices);
        return false;
    }
    ...
    LLAMA_LOG_INFO("%s: MoE cache ineligible: no host-resident routed expert tensors\n", __func__);
    return false;
}
