...
// Upstream auto is Ampere-only (8.0) and forced-on starts at Volta (7.0).
// This machine is V100 (sm_70) + V100 + P40 (sm_61). Q5_K MMVQ already
// requires __dp4a, so Pascal is the real floor for both auto and on.
static constexpr int    moe_cache_cc_forced_min               = GGML_CUDA_CC_DP4A;
static constexpr int    moe_cache_cc_ampere                   = GGML_CUDA_CC_AMPERE;
...
struct moe_cache_device {
    moe_cache_device(int logical, int physical, int compute_capability = 0)
        : logical(logical), physical(physical), compute_capability(compute_capability) {}
...
static int moe_cache_min_compute_capability(bool automatic) {
    // auto and on share the Pascal floor. Mixed Pascal/Volta sets still
    // serialize fills because minimum_capability < Ampere.
    (void) automatic;
...
// Volta+ runs cached MMVQ next to attention. On this board the V100s share a
// PEX x16 uplink to the host and the P40 is PCIe 3.0 x8, so Pascal is not used
// as an expert-cache device whenever a Volta GPU is present: fills would steal
// DDR4 bandwidth and serialize behind the slower link.
static bool moe_cache_is_primary_cache_device(int compute_capability) {
    return compute_capability >= GGML_CUDA_CC_VOLTA;
}

static bool moe_cache_cuda_has_primary_device() {
    const ggml_cuda_device_info & info = ggml_cuda_info();
    for (int i = 0; i < info.device_count; ++i) {
        if (moe_cache_is_primary_cache_device(info.devices[i].cc)) {
            return true;
        }
    }
    return false;
...
static int moe_cache_query_device(
    ...
    if (!moe_cache_is_primary_cache_device(cuda_device.cc) &&
        moe_cache_cuda_has_primary_device()) {
        return 0;
    }
...
// Estimate usable slab bytes without claiming the process-wide budget.
// Claiming every device during the auto floor check parked the P40's 21 GiB
// in accounting, then the MTP session saw 0 MiB and went dormant.
static bool moe_cache_probe_slab(
        moe_cache_session & session, moe_cache_device & device,
        size_t scratch_reserved, size_t & slab_limit) {
    slab_limit = 0;
    if (device.dead.load()) {
        return false;
    }
    if (device.budget_ready) {
        slab_limit = device.budget_limit > scratch_reserved
            ? device.budget_limit - scratch_reserved : 0;
        return device.budget_limit > 0;
    }
    ...
    size_t total_memory = 0;
    cudaError_t error = cudaMemGetInfo(&free_memory, &total_memory);
    if (!moe_cache_cuda_ok(device, error, "memory probe", false)) {
        return false;
    }

    const size_t reserve_bytes = session.config.reserve_mb << 20;
    size_t available = free_memory > reserve_bytes ? free_memory - reserve_bytes : 0;
    {
        std::lock_guard<std::mutex> lock(g_budget_mu);
        auto found = g_physical_budgets.find(device.physical);
        if (found != g_physical_budgets.end() &&
            found->second.outstanding_bytes > 0) {
            available = available > found->second.outstanding_bytes
                ? available - found->second.outstanding_bytes : 0;
        }
        if (session.config.budget_mb > 0) {
            available = std::min(available, session.config.budget_mb << 20);
        }
    }
    slab_limit = available > scratch_reserved ? available - scratch_reserved : 0;
    return available > 0;
}
...
static void * moe_cache_session_create(
    ...
    try {
        ...
        for (int index = 0; index < n_backends &&
            ...
            if (!moe_cache_is_primary_cache_device(capability) &&
                moe_cache_cuda_has_primary_device()) {
                MOE_CACHE_LOG("[moe-cache] CUDA%d skipped: Pascal expert cache disabled while Volta GPUs are present\n",
                        physical);
                continue;
            }
            ...
            session->devices.emplace_back(
                    new moe_cache_device(logical, physical, capability));
            MOE_CACHE_LOG("[moe-cache] CUDA%d selected: compute capability %d.%d\n",
                    physical, properties.major, properties.minor);
        }
    ...
    moe_cache_pool * pool = nullptr;
    {
        ...
        for (const auto & device_ptr : session->devices) {
            ...
            if (candidate.dead.load() ||
                !moe_cache_probe_slab(*session, candidate, reserved, slab_limit)) {
                continue;
            }
            budget_devices++;
        ...
        // This fork: one Pascal/Volta slab is enough. Upstream auto wanted two
        // Ampere GPUs; the MTP session here often only has the P40 left.
        if (budget_devices == 0 || slab_devices < 1) {
        ...
        auto route_weight = [&](moe_cache_device & candidate) -> size_t {
            ...
            const size_t reserved = moe_cache_scratch_total(
                    candidate.scratch_reserve, &scratch_requirements);
            size_t slab_limit = 0;
            if (!moe_cache_probe_slab(*session, candidate, reserved, slab_limit)) {
                return 0;
            }
            if (candidate.budget_ready) {
        ...
        for (const auto & device_ptr : session->devices) {
            if (moe_cache_is_primary_cache_device(device_ptr->compute_capability) &&
                route_weight(*device_ptr) > 0) {
                primary_available = true;
                break;
            }
        }
        auto assignment_weight = [&](moe_cache_device & candidate) -> size_t {
            const size_t weight = route_weight(candidate);
            if (weight == 0) {
                return 0;
            }
            if (primary_available &&
                !moe_cache_is_primary_cache_device(candidate.compute_capability)) {
                return 0;
            }
            return weight;
        };
        ...
        if (!selected) {
            for (const auto & device_ptr : session->devices) {
                const size_t weight = assignment_weight(*device_ptr);
        ...
        if (!selected && has_layer && !tensor_override) {
            ...
            for (const auto & device_ptr : session->devices) {
                moe_cache_device & candidate = *device_ptr;
                const size_t weight = assignment_weight(candidate);
                if (weight == 0) {
                    continue;
                }
                size_t assigned = 0;
        ...
        if (!moe_cache_prepare_budget(*session, *selected)) {
            return nullptr;
        }
