...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_GET_ROWS:
            {
                // Tiny gathers (decode-sized PLE / GPU-side rows) stay single-threaded: waking the
                // pool is slower than the copy. Large host tables with many rows (PLE prefill on a
                // 50+ GiB mmap) already shard by ith/nth in ops.cpp — use the pool there.
                {
                    const int64_t nr = node->src[1] ? ggml_nelements(node->src[1]) : 1;
                    const size_t src0_bytes = node->src[0] ? ggml_nbytes(node->src[0]) : 0;
                    if (nr >= 32 && src0_bytes > ((size_t) 64 << 20)) {
                        n_tasks = n_threads;
                    } else {
                        n_tasks = 1;
                    }
